...
F3DAUDIOAPI int F3DAudioCheckInitParams(
    ...
    /* Adrien: The docs don't clearly say it, but the debug dll does check that
...
static float ComputeConeParameter(
    ...
    if (angle <= halfOuterAngle)
    {
        ...
        /* Sooo... This is awkward. MSDN doesn't say anything, but X3DAudio.h
         * says that this should be lerped. However in practice the behaviour of
         * X3DAudio isn't a lerp at all. It's easy to see with big
         * (InnerAngle / OuterAngle) values. If we want accurate emulation,
         * we'll need to either find what formula they use, or use a more
         * advanced interpolation, like tricubic.
         *
...
typedef struct
{
    ...
    /* Not strictly necessary because it can be inferred from the SpeakerCount
     * field of the F3DAUDIO_HANDLE, but makes code much cleaner and less error
     * prone
     */
...
static void ComputeInnerRadiusDiffusionFactors(
    ...
    {
        /* Determined experimentally that this is indeed a linear law, with 100% confidence.
         * -Adrien
         */
        a = 1.0f - 2.0f * normalizedRadialDist;

        /* Lerping here is an approximation.
         * TODO: High accuracy version. Having stared at the curves long enough,
         * I'm pretty sure this is a quadratic, but trying to polyfit with numpy
         * didn't give nice, round polynomial coefficients...
         * -Adrien
         */
    ...
    else if (normalizedRadialDist <= 1.0f)
    {
        ...
        /* Similarly, this is a lerp based on the midpoint value; the real,
         * high-accuracy curve also looks like a quadratic.
         * -Adrien
         */
...
static inline void ComputeEmitterChannelCoefficients(
    ...
    if (diffusionFactors[DIFFUSION_SPEAKERS_MATCHING] > 0.0f)
    {
        ...
        /* Now, a critical point: We shouldn't be sending sound to matching
         * speakers when x and y are close to 0. That's the contract we get from
         * ComputeInnerRadiusDiffusionFactors, which checks that we're not too
         * close to the zero distance. This allows the atan2 calculation to give
         * good results.
         */
...
static inline void CalculateMatrix(
    ...
    /* Note: For both cone calculations, the angle might be NaN or infinite if
     * distance == 0... ComputeConeParameter *does* check for this special case.
     * It is necessary that we still go through the ComputeConeParameter
     * function, because omnidirectional cones might give either InnerVolume or
     * OuterVolume.
     * -Adrien
     */
    if (pListener->pCone)
    {
        /* Negate the dot product because we need listenerToEmitter in this case
         * -Adrien
         */
    ...
    else
    {
        ...
        /* Handling the mono-channel emitter case separately is easier than
         * having it as a separate case of a for-loop; indeed, in this case, we
         * need to ignore the non-relevant values from the emitter, _even if
         * they're set_.
         */
        if (pEmitter->ChannelCount == 1)
        {
        ...
        else /* Multi-channel emitter case. */
        {
            ...
            for (iEC = 0; iEC < pEmitter->ChannelCount; ++iEC)
            {
                ...
                {
                    /* First compute the emitter channel vector relative to the
                     * emitter base...
                     */
